...
export default function LanguageToggle({ scrolled = true }: LanguageToggleProps) {
  ...
  return (
    <button
      ...
      disabled={isPending}
      className={`h-11 px-4 flex items-center justify-center rounded-[var(--radius-xs)] border text-xs font-semibold tracking-[0.12em] transition-[background-color,border-color,color,transform] duration-200 ease-out active:scale-[0.96] disabled:opacity-50 ${
        scrolled
